...
def read_requences():
    ...
    with open('data/prior_needs.csv') as dataset_pointer:
        ...
        for i, row in enumerate(csv_reader):
            ...
            hour_block = row[5]
            prior_needs = row[6]
            s1_symbols = row[0:3] + [hour_block] + prior_needs.split()
            s1 = ' '.join(s1_symbols)
            # s1 = normalizeString(s1)

            current_needs = row[7]
            s2 = ' '.join(current_needs.split())
...
MAX_LENGTH = 29
base_folder = 'data/model/all'

if not os.path.isdir(base_folder):
    os.makedirs(base_folder)
if not os.path.isdir(base_folder + '/attention'):
    os.makedirs(base_folder + '/attention')
...
def save_model(encoder: EncoderRNN, decoder: AttnDecoderRNN, encoder_optimizer, decoder_optimizer, iteration=None, loss=None):
    ...
    torch.save(encoder_state, base_folder + '/encoder_' + str(iteration) + '_' + str(loss) + ".tar")

    # save decoder model
    decoder_state = {'epoch': iteration, 'state_dict': decoder.state_dict(),
                     'optimizer': decoder_optimizer.state_dict(), 'loss': loss, }
    torch.save(decoder_state, base_folder + '/decoder_' + str(iteration) + '_' + str(loss) + ".tar")
######################################################################
...
def trainIters(encoder: EncoderRNN, decoder: AttnDecoderRNN, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    with open(base_folder + '/' + filename + '.csv', 'w') as loss_file:
    ...
    eval_seq.showPlot(plot_losses, force_show=False, save_figure=True, figure_filename=base_folder + '/' + filename + '.png')
...
eval_seq.evaluate_data(encoder1, attn_decoder1, houston, base_folder)
